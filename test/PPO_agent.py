...
class ActorCritic(nn.Module):
    ...
    def act(self, state):
        ...
        action_std = self.actor_log_std.exp() # 标准差，这里可以用网络输出，但是先不改
        action_var = action_std.pow(2) # 方差
        cov_mat = torch.diag_embed(action_var.expand_as(action_mean)) # 协方差矩阵
        
        # 创建多维高斯分布
        dist = MultivariateNormal(action_mean, cov_mat) # 构建多维高斯分布
        
        # 采样得到动作
        action = dist.sample()
        action_logprob = dist.log_prob(action) # 计算这个策略的概率的对数
        state_val = self.critic(state) # 基于这个策略估计状态价值
    ...
    def evaluate(self, state, action):
        ...
        action_std = self.actor_log_std.exp() # 标准差，这里可以用网络输出，但是先不改
        action_var = action_std.pow(2) # 方差
        cov_mat = torch.diag_embed(action_var.expand_as(action_mean))
...
class PPO_Agent:
    def __init__(self, state_dim=12, action_dim=6, lr_actor=0.0003, lr_critic=0.001, gamma=0.99, K_epochs=40, eps_clip=0.2):
        self.gamma = gamma # 未来衰减奖励核心
        self.eps_clip = eps_clip # clip范围
        self.K_epochs = K_epochs # 一批数据重复训练K次
        ...
        self.optimizer = torch.optim.Adam([
            ...
            {'params': [self.policy.actor_log_std], 'lr': lr_actor}, # 这里的std计算可能有问题
    ...
    def select_action(self, prev_angles, target_pos, obs_pos):
        ...
        with torch.no_grad(): # 采样阶段，不需要梯度计算
    ...
    def update(self):
        ...
        rewards = []
        discounted_reward = 0
        for reward, is_terminal in zip(reversed(self.buffer.rewards), reversed(self.buffer.is_terminals)): # 什么时候储存的rewards？
            if is_terminal:
                discounted_reward = 0
        ...
        # 计算 Advantages
        # 这里的代码有问题，rewards已经归一化了，old_state_values没有归一化
